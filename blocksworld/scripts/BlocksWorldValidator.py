...
class BlocksWorldValidator:
    ...
    def compute_performance_metrics(self, predictions: List[List[List[int]]], 
                                 targets: List[List[List[int]]], 
                                 goal_states: List[List[int]]) -> Dict[str, float]:
...
if __name__ == "__main__":
    ...
    valid_state = [1, 0,  # A,B on table
                   0, 1,  # A on B, B on A
                   0, 1,  # A,B clear
                   0, 0]  # A,B held
    
    # Invalid state: A floating
    invalid_state = [0, 0,  # A,B on table
                    0, 0,  # A on B, B on A
                    1, 1,  # A,B clear
                    0, 0]  # A,B held
